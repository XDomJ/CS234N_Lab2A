...
namespace MMABooksDBClasses
{
    public static class CustomerDB
    {
        ...
        public static bool DeleteCustomer(Customer customer)
        {
            ...
            deleteCommand.Parameters.RemoveAt(customer.CustomerID); 
            deleteCommand.Parameters.RemoveAt(customer.Name);
            deleteCommand.Parameters.Remove(customer.Address);
            deleteCommand.Parameters.Remove(customer.City);
            deleteCommand.Parameters.Remove(customer.State);
            deleteCommand.Parameters.Remove(customer.ZipCode);  //recheck all of this
        ...
        {
            MySqlConnection connection = MMABooksDB.GetConnection(); // create a connection
            ...
            try
            {
                ...
                MySqlDataReader custReader = updateCommand.ExecuteReader(CommandBehavior.SingleRow);// execute the command
                if (custReader.Read())
                {
                    return true;// if the number of records returned = 1, return true otherwise return false
                }
                else
                    return false;
            }
